...
def register():
    ...
    if request.method == 'POST':
        ...
        confirm_password = request.form.get('confirm_password')   # ← from your HTML
        ...
        try:
            ...
            if USE_POSTGRES:
                cur = db_execute(conn,
                    "INSERT INTO users (name, email, password, role, is_verified) VALUES (%s, %s, %s, %s, %s) RETURNING id",
                    (name, email, hashed_store, role, is_verified))
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                ...
                if 'student_name' not in df.columns:
                    flash('File must have a column: student_name.', 'danger')
                    return redirect(url_for('upload_students'))
...
def student_dashboard():
    ...
    exams = db_execute(conn, """
        ...
        FROM exams e
        JOIN subjects s ON e.subject_id = s.id
        JOIN allowed_students a ON a.subject_id = s.id AND a.student_name = ?
    ...
    """, (student_name, student_id, student_id)).fetchall()
    ...
    key_exams = db_execute(conn, """
        ...
        AND s.id NOT IN (
            SELECT subject_id FROM allowed_students WHERE student_name = ?
        )
    """, (student_id, student_id, student_name)).fetchall()
